            ...
            if (selectedVehicleId) {
                ...
                switch (activeTab) {
                    case 'veiculos':
                        ...
                        `;
                        // Event listener for the new form
    ...
    appContent.innerHTML = `
        ...
        <div class="bg-white p-6 rounded-lg shadow table-container">
            ...
            <div class="flex justify-between items-center mt-4">
                <button onclick="window.prevPage()" class="px-4 py-2 bg-gray-300 rounded ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''}">Anterior</button>
                <span>Página ${currentPage} de ${totalPages || 1}</span>
                <button onclick="window.nextPage()" class="px-4 py-2 bg-gray-300 rounded ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''}">Próxima</button>
            </div>
        </div>
    `;
    break;
        ...
        window.handleCompleteMaintenance = handleCompleteMaintenance; // ADICIONE ESTA LINHA
